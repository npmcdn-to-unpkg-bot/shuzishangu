...
$app->put(
    ...
    function () use ($app){
        try {
            ...
            $log_info = 'set auth switch success';
            ...
            $log_info = 'set auth swtich:'.$e->getMessage();
...
$app->put(
    ...
    function () use ($app){
        try {
            ...
            $log_info = 'set auth type success';
            exec("logger -t web -p 5 ".$log_info);  
            ...
            $log_info = 'set auth type:'.$e->getMessage();
            exec("logger -t web -p 3 ".$log_info);      
        }
    }
); 
...
function import_user($arr){
    ...
    try {
        ...
        $log_info = 'loadingin user success';
        exec("logger -t web -p 5 ".$log_info);  
    }catch(Exception $e) {
        $log_info = 'import user:'.$e->getMessage();
        exec("logger -t web -p 3 ".$log_info);
    }
    return $success;
}
...
$app->get(
    ...
    function (){
        try {
            ...
            fclose($file);
            $log_info = "download configration of csv successfully !";
            exec("logger -t web -p 5 ".$log_info);
        }catch(Exception $e) {
            $error = array(
                'success' => false,
...
$app->put(
    ...
    function () use ($app){
	    ...
	    $access_code = $crypto->decode($access_code);
            ...
            $log_info = 'add accesscode:'.$e->getMessage();
            exec("logger -t web -p 3 ".$log_info);      
        }
    }
); 
...
$app->delete(
    ...
    function ($accesscode){
        try {
            ...
            $log_info = 'delete accesscode:'.$e->getMessage();
            exec("logger -t web -p 3 ".$log_info);      
        }
    }
); 
...
$app->put(
    ...
    function () use ($app){
        try {
            ...
            $log_info = 'restore default portal :'.$e->getMessage();
            exec("logger -t web -p 3 ".$log_info);      
        }
    }
);
...
$app->get(
    ...
    function (){
        $tftpinfo = array();
        $cmd = "uci get tftplog.tftp.turn_on_off";
        ...
        $cmd = "uci get tftplog.tftp.ip_auto";
        ...
        $cmd = "uci get tftplog.tftp.timer";
...
$app->put(
    ...
    function () use ($app){
        try {
            ...
            $cmdswitch = "uci set tftplog.tftp.turn_on_off=".$TftpStatus.";";
            $cmdcom = "uci commit tftplog";
            $cmd = $cmdswitch.$cmdcom;
            exec($cmd, $out, $return_var);
            if(0 === $return_var){
                $response = array(
                            'success' => true,
            ...
            $log_info = 'set tftp server swtichh success';
            exec("logger -t web -p 5 ".$log_info);  
            ...
            $log_info = 'set tftp server swtich:'.$e->getMessage();
            exec("logger -t web -p 3 ".$log_info);      
        }
    }
); 
...
$app->put(
    ...
    function () use ($app){
        try {
            ...
            $cmdlog = "uci set tftplog.tftp=log;";
            $cmdip = "uci set tftplog.tftp.ip_auto=".$ip.";";
            $cmdcycle = "uci set tftplog.tftp.timer=".$cycle.";";
            $cmdcom = "uci commit tftplog";
            $cmd = $cmdlog.$cmdip.$cmdcycle.$cmdcom;
            exec($cmd, $out, $return_var);
            ...
            echo json_encode($response);
            $log_info = 'set TFTP info success!';
            exec("logger -t web -p 5 ".$log_info);  
        }catch(Exception $e) {
            $error = array(
                    'success' => false,
            ...
            $log_info = 'set TFTP info:'.$e->getMessage();
            exec("logger -t web -p 3 ".$log_info);      
        }
    }
); 
...
$app->put(
    ...
    function () use ($app){
        try {
            ...
            $cmdlog = "uci set tftplog.tftp=log;";
            $cmdip = "uci set tftplog.tftp.ip_manu=".$ip.";";
            $cmdcom = "uci commit tftplog";
            $cmd = $cmdlog.$cmdip.$cmdcom;
            exec($cmd, $out, $return_var);
