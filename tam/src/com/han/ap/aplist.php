...
$app->put(
	...
	function (){
		try {
			$cmd_flag="echo 06 > /proc/kes_reboot_flag";
			shell_exec($cmd_flag);
			$cmd="reboot";
        	...
        	$error = array(
        		...
        		'msg' => 'reboot ap error');
        	echo json_encode($error);
        	$log_info = "reboot ap: ".$e->getMessage();
		...
		try {
			...
			exec($cmd,$out,$ret_err);
				if($ret_err){
					...
					$log_info ="modify the name of the AP failed";
				}else{
					$response=array(
						'success'=>true,
						'result'=>"success"
					);
					$log_info ="modify the name of the AP successfully";
				}
			echo json_encode($response);
			exec("logger -t web -p 5 \"".$log_info."\"");
		}catch(Exception $e) {
			...
			echo json_encode($error);
			$log_info = "modify the name of the AP: ".$e->getMessage();
            exec("logger -t web -p 3 \"".$log_info."\"");
		}
	});
		...
		try {
			...
			exec($cmd,$out,$ret_err);
					if($ret_err){
						...
						$log_info ='modify the location of the ap failed';
					}else{
						$response=array(
							'success'=>true,
							'result'=>"success"
						);
						$log_info ='modify the location of the ap successfully';
					}
			echo json_encode($response);
			exec("logger -t web -p 5 \"".$log_info."\"");
        }catch(Exception $e) {
        	...
        	echo json_encode($error);
        	$log_info = "modify the location of the ap: ".$e->getMessage();
            exec("logger -t web -p 3 \"".$log_info."\"");
        }
	});
		...
		function() use($app){
			try {
				...
				if($ret_err){
					...
					$log_info ="the ap which mac is ".$requestJson->mac." access the Group failed !";
				}else{
					$response=array(
						'success'=>true,
						'result'=>$ret_err
					);
					$log_info ="the ap which mac is ".$requestJson->mac." access the Group successfuly !";
				}
				echo json_encode($response);
				exec("logger -t web -p 5 \"".$log_info."\"");
            }catch(Exception $e) {
            	$error = array(
            		...
            		'msg' => 'accept ap  error');
            	echo json_encode($error);
            	$log_info = "the ap which mac is ".$requestJson->mac." access the Group: ".$e->getMessage();
                exec("logger -t web -p 3 \"".$log_info."\"");
            }
});
		...
		function() use($app){
			try {
    			...
    			if($ret_err){
    				...
    				$log_info ="kick the ap which mac is ".$requestJson->mac." out the Group failed !";
    			}else{
    				$response=array(
    					'success'=>true,
    					'result'=>$ret_err
    				);
    				$log_info ="kick the ap which mac is ".$requestJson->mac." out the Group successfully !";
    			}
    			echo json_encode($response);
    			exec("logger -t web -p 5 \"".$log_info."\"");
			}catch(Exception $e) {
				...
				echo json_encode($error);
				$log_info = "kick the ap which mac is ".$requestJson->mac." out the Group: ".$e->getMessage();
                exec("logger -t web -p 3 \"".$log_info."\"");
			}
});
...
$app->put(
		...
		function() use($app){
			try {
				...
				echo json_encode($error);
				$log_info = "delete the ap which mac is ".$requestJson->mac." from the group: ".$e->getMessage();
                exec("logger -t web -p 3 \"".$log_info."\"");
			}
});
...
$app->put(
	...
	function() use($app){
		try {
			...
			echo json_encode($error);
			$log_info = "set cluster_priority=".$mac." :".$e->getMessage();
            exec("logger -t web -p 3 \"".$log_info."\"");
		}
});
...
$app->put(
		...
		function() use($app){
			try {
    			...
    			$log_info ="modify ip info of the ap successfully !";
    			echo json_encode($response);
    			exec("logger -t web -p 5 \"".$log_info."\"");
            }catch(Exception $e) {
            	$error = array(
            	...
            	echo json_encode($error);
            	$log_info = "modify ip info of the ap: ".$e->getMessage();
                exec("logger -t web -p 3 \"".$log_info."\"");
            }
});
$app->get(
	...
	function (){
		try {
    		$reload = 'ubus call network reload > /dev/null &';
    		exec($reload);
    		$response=array(
    			'success'=>true,
    			'result'=>"success"
        	...
        	echo json_encode($error);
        	$log_info = "restart network: ".$e->getMessage();
            exec("logger -t web -p 3 \"".$log_info."\"");
        }
	});
	...
	function (){
		try {
        	...
        	$error = array(
        		...
        		'msg' => 'cfgclr_error');
        	echo json_encode($error);
        	$log_info = "clear configration of the ap: ".$e->getMessage();
            exec("logger -t web -p 3 \"".$log_info."\"");
        }
	});
/*backup all cfg*/
$app->get(
	...
	function (){
		try {
        	...
        	echo json_encode($error);
        	$log_info = "backup the configration of ap: ".$e->getMessage();
            exec("logger -t web -p 3 \"".$log_info."\"");
        }
	}
);
	...
	function($filename) use($app){
		try {
        	...
        	$error = array(
        		...
        		'result'=>'103');
        	echo json_encode($error);
			$log_info = "resotre the configration of ap: ".$e->getMessage();
			exec("logger -t web -p 3 \"".$log_info."\"");
        }
	});
/*update  firmware*/
$app->post(
	...
	function($filename) use($app){
		try {
        	...
        	echo json_encode($error);
        	$log_info = "upload the version of ap: ".$e->getMessage();
            exec("logger -t web -p 3 \"".$log_info."\"");
        }
	});
	...
	function() use($app){
		try {
        	...
        	$error = array(
        		...
        		'result' => $result_array );
        	echo json_encode($error);
        	$log_info = "upload the version of ap by url: ".$e->getMessage();
            exec("logger -t web -p 3 \"".$log_info."\"");
        }
});
			...
			try {
				...
				switch($type){
					...
					default:
						...
						echo json_encode($response);
						$log_info = "modify the status of led failed: input parameter error !";
						exec("logger -t web -p 5 \"".$log_info."\"");
						return;
				}
