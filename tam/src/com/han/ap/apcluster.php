...
$app->get(
		...
		function(){
			try {
				$result_array = array();
    			$com_key=array("cluster_id","cluster_name","cluster_location","cluster_vip","Viewer","GuestOperator");
...
$app->put(
		...
		function() use($app){
			try {
				...
				echo json_encode($error);
				$log_info = "restart Group: ".$e->getMessage();
...
$app->put(
		...
		function() use($app){
			try {
				...
				$log_info = "modify group id: ".$e->getMessage();
				exec("logger -t web -p 3 ".$log_info);
				echo json_encode($error);
			}
...
$app->put(
		...
		function() use($app){
			try {
            	...
            	$log_info = "modify Group info: ".$e->getMessage();
                exec("logger -t web -p 3 ".$log_info);
            	echo json_encode($error);
            }
...
$app->put(
		...
		function() use($app){
			try {
     			$requestBody=$app->request()->getBody();
     			$requestJson=json_decode($requestBody);
     			$cmd="cluster_cli vip ".$requestJson->cluster_vip;
     			exec($cmd,$lines,$ret_err);
     			if($ret_err){
     				$response=array(
     					'success'=>false,
            	...
            	$log_info = "modify Group vip: ".$e->getMessage();
                exec("logger -t web -p 3 ".$log_info);
            	echo json_encode($error);
            }
...
$app->put(
		...
		function() use($app){
			try {
				...
				$log_info = "Modify user info: ".$e->getMessage();
				 exec("logger -t web -p 3 ".$log_info);
				echo json_encode($error);
			 }
		});
