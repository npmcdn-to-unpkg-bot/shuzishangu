 ...
 */
.factory('batchSyncConfig', ['$translate', '$q', 'lockScreen', 'task', 'toastr', 'InterService', 'operationLog',
	function($translate, $q, lockScreen, task, toastr, InterService, operationLog) {
...
.service('InterService',['authentifiedRequest', '$cookieStore', 'clientService', function(authentifiedRequest, $cookieStore, clientService) {
	...
	vm.setInterApListInfo = function(x) {
			vm.interaplistinfo = x;
		}
	...
	vm.pattern = {
		...
		"maxClients":"/^(^[1-9]\\d{0,1}$)|(^1[0,1]\\d$)|(^12[0-8]$)$/",
...
.directive("repeat", [function () {
	...
	return {
		...
		link:function(scope,ele,attrs){
			ele.bind('paste',function(attrs){
				event.returnValue = false;
			});
			ele.bind("keydown",function(attrs){
				if(event.keyCode == 13) {
					event.keyCode = 9;
				}
			});
