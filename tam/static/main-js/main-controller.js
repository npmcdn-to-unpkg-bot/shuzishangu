angular.module('main')
	.run(['$rootScope','$interval','toastr','InterService','authentifiedRequest','permissions','clientService',function($rootScope,$interval,toastr,InterService,authentifiedRequest,permissions,clientService) {
		        	...
		        	for(var i=data.result.length; i> 0 ;i-- ){
						...
						if(!flag){
							if(data.result[i-1].role==1){
								InterService.setPVCinfo(data.result[i-1]);
								flag=true;
							}
						}
		            }
		...
		$interval(function() {
			authentifiedRequest.post('/authentication/sessionStatus', null,null, function(){}, function(){});
		}, 10000);
	...
	.controller('mainController', ['$scope','$translate', 'singleModal', 'InterService', '$interval', '$translate','ClusterService', 'monitorService','$cookieStore','operationLog', 'APService', function($scope,$translate, singleModal, InterService, $interval, $translate,ClusterService, monitorService,$cookieStore,operationLog, APService) {
